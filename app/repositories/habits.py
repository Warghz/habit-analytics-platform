...
class HabitRepository:
    ...
    @staticmethod
    async def create_habits(session: AsyncSession, user_id: int, data: HabitCreate) -> Habit:
    ...
    @staticmethod
    async def get_habits_by_id(session: AsyncSession, habit_id: int, user_id: int):
        result = await session.execute(
            select(Habit).where(
                Habit.id == habit_id,
